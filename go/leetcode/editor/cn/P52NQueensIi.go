//n 皇后问题研究的是如何将 n 个皇后放置在 n×n 的棋盘上，并且使皇后彼此之间不能相互攻击。
//
//
//
// 上图为 8 皇后问题的一种解法。
//
// 给定一个整数 n，返回 n 皇后不同的解决方案的数量。
//
// 示例:
//
// 输入: 4
//输出: 2
//解释: 4 皇后问题存在如下两个不同的解法。
//[
// [".Q..",  // 解法 1
//  "...Q",
//  "Q...",
//  "..Q."],
//
// ["..Q.",  // 解法 2
//  "Q...",
//  "...Q",
//  ".Q.."]
//]
//
//
//
//
// 提示：
//
//
// 皇后，是国际象棋中的棋子，意味着国王的妻子。皇后只做一件事，那就是“吃子”。当她遇见可以吃的棋子时，就迅速冲上去吃掉棋子。当然，她横、竖、斜都可走一或 N
//-1 步，可进可退。（引用自 百度百科 - 皇后 ）
//
// Related Topics 回溯算法

package main

//Go：N皇后 II
import (
	"fmt"
)

func main() {
	fmt.Println(totalNQueens(4))
}

//leetcode submit region begin(Prohibit modification and deletion)
var solutions2 int

func totalNQueens(n int) int {
	solutions2 = 0
	column := make([]bool, n)
	diagonals1 := make([]bool, 2*n-1)
	diagonals2 := make([]bool, 2*n-1)

	backtrack2(0, n, column, diagonals1, diagonals2)
	return solutions2
}

func backtrack2(row, n int, column, diagonals1, diagonals2 []bool) {
	if row == n {
		solutions2 += 1
		return
	}

	for i := 0; i < n; i++ {
		//foreach diagonals array
		diagonal1 := row + n - 1 - i
		diagonal2 := row + i

		if column[i] || diagonals1[diagonal1] || diagonals2[diagonal2] {
			continue
		}
		column[i] = true
		diagonals1[diagonal1], diagonals2[diagonal2] = true, true
		backtrack2(row+1, n, column, diagonals1, diagonals2)
		column[i] = false
		diagonals1[diagonal1], diagonals2[diagonal2] = false, false
	}
}

//leetcode submit region end(Prohibit modification and deletion)
